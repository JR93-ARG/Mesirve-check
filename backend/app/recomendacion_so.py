...
# Asesoramiento integral según el hardware detectado. Las opciones de
# sistema operativo salen del catálogo en la base (tabla
# sistemas_operativos) — así alguien puede sumar un Linux liviano nuevo
# sin tocar código, vía el panel de administración. El resto (qué
# programas soportaría, navegador recomendado, optimizaciones) sigue
# siendo una regla simple por nivel, no requiere catálogo propio todavía.
...
async def evaluar_sistema_operativo(db, ram_gb, tipo_almacenamiento, cpu_puntaje):
    notas = []
    ...
    query = text("""
        SELECT nombre, tipo, liviano, ram_minima, ram_recomendada, requiere_cpu_moderno,
               pros, contras, notas AS nota_extra, url_referencia
        FROM sistemas_operativos
        WHERE ram_minima IS NULL OR ram_minima <= :ram_gb
        ORDER BY liviano ASC, ram_minima DESC NULLS LAST
    """)
    filas = (await db.execute(query, {"ram_gb": ram_gb})).mappings().all()
    opciones = [dict(f) for f in filas if not (f["requiere_cpu_moderno"] and not compatible_win11)]
